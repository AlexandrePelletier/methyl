...
fwrite(peaks_hsc_lga,fp(out,"differential_peaks_accessibility_lga_vs_ctrl_hsc.csv.gz"))
...
hsc_lga_tf<-FindMotifs(object = atacs,
           assay = "lin_peaks",
           features = peaks_hsc_lga_xy[p_val_adj<0.001&avg_log2FC>0]$peak )
hsc_lga_tf_down<-FindMotifs(object = atacs,
           assay = "lin_peaks",
           features = peaks_hsc_lga_xy[p_val_adj<0.001&avg_log2FC<0]$peak )
hsc_lga_tf_down
hsc_lga_tf_all<-rbind(data.table(hsc_lga_tf)[,enriched_in:="lga"],data.table(hsc_lga_tf_down)[,enriched_in:="ctrl"])
hsc_lga_tf_all[order(pvalue)][1:20]
fwrite(peaks_hsc_lga,fp(out,"differential_peaks_accessibility_lga_vs_ctrl_hsc.csv.gz"))

peaks_hsc_lga_xy[p_val_adj<0.001&avg_log2FC<0]
MotifPlot(
  object = atacs,
