...
out<-"outputs/17-LGA_vs_Ctrl_EGR1_interaction/ctrl_runs"
dir.create(out)
...
tfg_lga<-Reduce(rbind,lapply(39:50, function(i){
  print(i)
  seed<-i
  set.seed(seed)
  mtd[group=="ctrl",cells_choosed:=bc%in%sample(bc,50,replace = T),by=.(sample_hto,cell_type_hmap)] #50 cells max by sample_hto and cell_type
...
fwrite(tfg_lga,fp(out,"tf_targets_interactions_50_ctrl_runs_5kcells.csv.gz"))
