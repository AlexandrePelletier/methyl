...
out<-"outputs/17-LGA_vs_Ctrl_EGR1_interaction/sample_runs"
dir.create(out)
...
tfgs<-Reduce(rbind,lapply(unique(mtd$sample_hto), function(s){
  print(s)

  cbpsf<-subset(cbps,sample_hto==s)
  scenics<-RunScenic(cbpsf,
                     dir = fp(out,paste0("run_",s)),
                     run.steps = 1,
                   cisTarget_dir = "/disks/DATATMP/PhD_AlexandrePelletier/marcos/ref/cisTarget_databases/human/")

  tfg<-data.table(readRDS(fp(out,paste0("run_",s),"int/1.4_GENIE3_linkList.Rds")))
  return(tfg[,sample:=s])

  
  }))

fwrite(tfgs,fp(out,"tf_targets_interactions_by_samples.csv.gz"))
